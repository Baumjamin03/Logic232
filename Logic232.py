...
import time
import ssd1306
from communication import commUART, commSPI, commI2C
import CDGFormula
import sdcard
import uos
...
class SDCardModule: #SD-cards communicate over the spi protocoll
  def __init__(self, _cd, _mosi, _miso, _clk, _cs):  #All parameters expect a Pin Object
    self.cSPI = commSPI(_mosi, _miso, _clk)
    self.cs = _cs
...
class CDGSensor:  #class for the CDG connected via RS232C
  def __init__(self, id=1, baud=9600, _rx=16, _tx=17):  
    self.cUART = commUART(id, baud, _rx, _tx)
    self.temp = 0
    self.atm = 0
    self.pressure = 0
    self.hb = 0
...
myCDG = CDGSensor(_rx=16, _tx=17)     #object for vacuum gauge
SDCard = None
netClock = None
...
CAL_TEMP = bytes([0x03, 0x10, 0x31, 0x01, 0x42])        #command to switch to temperaure mode  
CAL_ATM = bytes([0x03, 0x10, 0x31, 0x02, 0x43])         #command to switch to temperaure mode
HEATBLOCK_TEMP = bytes([0x03, 0x10, 0x31, 0x04, 0x45])  #command to switch to temperaure mode
ANALOG_OUT = bytes([0x03, 0x10, 0x31, 0x03, 0x44])      #command to switch to temperaure mode
...
while True:     #main loop
  ...
  if logSwitch.value():    
    ...
    while netClock == None:
      try:
        netClock = communication.netTime("INFICON-Guest", "") #WLAN connection for timestamp
        netClock.SetRTC()
      except OSError as e:    #error catching to avoid freeze
        Display.fill()
